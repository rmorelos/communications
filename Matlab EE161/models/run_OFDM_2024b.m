...
% EE 161 - Spring 2011 - San Jose State University. Modified Spring 2025
...
for n = 1:length(SNRVec)
    SNR = SNRVec(n);
    sim('OFDM_system_QPSKmod_2024b');
    ser(n) = ErrorVec(1);
    fprintf('%5.2f\t%e\n',SNR,ser(n));
end;
semilogy(SNRVec,ser,'b-s')
axis tight, grid on, xlabel('E/N_0 (dB)'), ylabel('SER')
axis([SNRVec(1) SNRVec(length(SNRVec)) 1e-5 5e-1])
