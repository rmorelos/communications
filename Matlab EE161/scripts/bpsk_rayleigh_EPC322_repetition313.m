...
% of the (time) diversity achieved with a binary even parity-check 
% (3,2,2) code and a binary repetition (3,1,3) code.
%
% Ideal interleaving (i.e., very large block interveaver length) is assumed
...
N = 1.7e+6;               % Maximum number of codewords sent per E/No value
...
fprintf('\nThis script will take several minutes to finish. Please be patient ...\n\n');
fprintf('-----------------------------------------------------------------\n');
...
title('BPSK modulation under flat Rayleigh fading with EPC(3,2,2) and repetition (3,1,3) codes');
legend('BPSK', 'Theory BPSK', ...
    'EPC(3,2,2)', 'Approximation EPC(3,2,2)', ...
    'Hamming(3,1,3)', 'Approximation Hamming(3,1,3)')

axis([snr(1) snr(length(snr)) 1e-5 5e-2]), grid on, hold off