% File: mappings_corr_MF.m
% Illustration of pulse-shaping schemes and correlator/matched filter 
% outputs in the absence of noise.
% EE 161. Spring 2016. San Jose State University.
% Modified for EE251 in Fall 2020, 2021, 2024
...
% (h) Unipolar Manchester
y8 = [bit(1)*A*y_M zeros(1,M)];
for i=2:length(bit)
    ytemp = [zeros(1,(i-1)*M) bit(i)*A*y_M];
    y8 = y8 + ytemp;
    y8 = [y8 zeros(1,M)];
end
...
% -------------------------------------
% Correlator and matched-filter outputs
% -------------------------------------
...
for i=1:length(bit)
    ...
    for j=(i-1)*M+1:i*M
        corr_UM(j) = sum;
        s_UM(j) = bit(i)*p_M(j-(i-1)*M);
        sum = sum + bit(i)*p_M(j-(i-1)*M)*p_M(j-(i-1)*M)/M;
    end
end
% -------------------------------------
% Plot signals
...
set(gca,'YTick',[0 1])
title('Example of pulse shaping waveforms')
subplot(4,1,2)
...
subplot(4,1,4)
plot(t,s_UM,'k'); ylabel ('Unipolar Manchester'); axis ([ 0 T_bits  -1.1*A 1.1*A ]), grid on
...
subplot(4,1,4)
plot(t,corr_UM,'k'); ylabel ('Unipolar Manchester'); axis ([ 0 T_bits  -1.1*A 1.1*A ]), grid on

% figure(3)
% subplot(3,1,1)
% plot(t,bit_M,'k'); ylabel ('Bits'); axis ([ 0 T_bits  -0.1 1.1 ]), grid on
% set(gca,'YTick',[0 1])
% title('Bit sequence [ 1 0 0 1 1 0 1 0]')
% subplot(3,1,2)
% plot(t,s_UNRZ,'k'); ylabel ('Unipolar NRZ'); axis ([ 0 T_bits 0 1.1*A ]), grid on
% subplot(3,1,3)
% plot(t,y1(1:length(y1)-2*M),'k'); ylabel ('MF output'); axis ([ 0 T_bits  0 1.1*A ]), grid on

% PLOT polar NRZ and unipolar Manchester waveforms
figure(3)
subplot(4,1,1)
plot(t,bit_M,'k'); ylabel ('Bits'); axis ([ 0 T_bits  -0.1 1.1 ]), grid on
set(gca,'YTick',[0 1])
...
subplot(4,1,4)
plot(t,y6(1:length(y8)-2*M),'k'); ylabel ('MF output'); axis ([ 0 T_bits -1.1*A 1.1*A ]), grid on
...
plot(t,bit_M,'k'); ylabel ('Bits'); axis ([ 0 T_bits  -0.1 1.1 ]), grid on
set(gca,'YTick',[0 1])
title('Unipolar Manchester')
subplot(4,1,2)
plot(t,s_UM,'k'); ylabel ('Unipolar Manchester'); axis ([ 0 T_bits -1.1*A 1.1*A ]), grid on
subplot(4,1,3)
plot(t,corr_UM,'k'); ylabel ('Correlator'); axis ([ 0 T_bits -1.1*A 1.1*A ]), grid on
subplot(4,1,4)
plot(t,y8(1:length(y8)-2*M),'k'); ylabel ('MF output'); axis ([ 0 T_bits -1.1*A 1.1*A ]), grid on
