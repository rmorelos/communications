% File: bpsk_rayleigh_2branch_diversity
% Comparison of combining techniques using two diversity branches,
...
for m=1:size(snr,2)
    ...
    for n=1:N
        ...
        recSC = r(sel);
        aveSC(m) = aveSC(m)+recSC^2/D;            % Received S+N with SC
        % Detector
        ...
        aveMRC(m) = aveMRC(m)+recMRC^2/D;       % Received S+N with MRC
        % Detector
        ...
        aveEGC(m) = aveEGC(m)+recEGC^2/D;       % Received S+N with EGC
        % Detector
...
ylabel('Bit Error Rate'); xlabel('Average E_s/N_0 (dB)'); legend('SC','EGC','MRC')
axis tight, grid on, hold off
title('Performance of BPSK with two-branch diversity')
figure(2)
plot(snr,10*log10(aveMRC/N),'-ok'), hold on, plot(snr,10*log10(aveEGC/N),'-^k'), 
plot(snr,10*log10(aveSC/N),'-sk'), grid on, hold off
xlabel('Average E_s/N_0 per branch (dB)'), ylabel('Post-combiner average E_s+N_0 (dB)')
legend('MRC','EGC','SC')
axis tight
