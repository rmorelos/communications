% File: pulse_shaping3.m
% Description: Illustration of various mappings and pulse shapes
% For the students of EE 160. Fall 2006. San Jose State University.
% Modified Fall 2012 and 2021. 

clear
seed = mod(1e6*now, 100000);
rng(seed,'twister');                    % Not the same seed at start

A = 1;                                  % Pulse amplitude
T = 1;                                  % Bit duration

bit = round(rand(1,16));                % Bit sequence
...
set(gca,'YTick',[0 1])
grid on
title('Examples of mapping and pulse shaping - EE160 - SJSU   \copyright 2021')
subplot(5,1,2)
plot(t,s_URZ,'k'); ylabel ('Unipolar RZ'); axis ([ 0 T_bits  -0.1*A 1.1*A ]);
grid on
subplot(5,1,3)
plot(t,s_PNRZ,'k'); ylabel ('Polar NRZ'); axis ([ 0 T_bits  -1.1*A 1.1*A ])
grid on
subplot(5,1,4)
